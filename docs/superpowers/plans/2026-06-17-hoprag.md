...
def smoke():
    """Wiring check with fakes — no network, no CLI."""
    from tests.conftest import FakeRetriever, FakeClaude  # type: ignore
    from hoprag.types import Chunk
    from hoprag.dataset import Example
    c = [Chunk(id="c1", title="Tim Burton", text="filmmaker born in Burbank")]
    retr = FakeRetriever({"": c})
...
Expected: prints `smoke OK: Burbank calls: 5` (exit 0).
